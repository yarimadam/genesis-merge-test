...
{
    public class CommMailConsumer : ConsumerBase
    {
        public CommMailConsumer() : base(ConfigurationManager.KafkaSettings.Topics[Topics.CommMail].GroupId)
        {
        }

        public async Task Listen()
        {
            var settings = ConfigurationManager.KafkaSettings.Topics[Topics.CommMail];
        ...
        public bool HandleOnMessage(Communication data)
        {
            try
            {
                CommunicationManager.Mail.Send(data);
            }
            catch (Exception ex)
            {
